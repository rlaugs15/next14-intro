# 넥스트JS 기본 학습 목차

[넥스트JS 공식문서(한글판)](https://nextjs-ko.org/docs)

## [#2 라우팅](/docs/routing.md)

- [#2.1 Routes 정의](/docs/routing.md/#21-routes-정의)
- [#2.2 Not Found Routes](/docs/routing.md/#22-not-found-routes)
- [#2.3 SSR vs CSR](/docs/routing.md/#23-ssr-vs-csr)
- [#2.4 Hydration](/docs/routing.md/#24-hydration)
- [#2.5 'use client'(중요)](/docs/routing.md/#25-use-client중요)
- [#2.6 요약](/docs/routing.md/#26-요약)
- [#2.7 Layouts](/docs/routing.md/#27-layouts)
- [#2.8 라우트 그룹과 메타데이터](/docs/routing.md/#28-라우트-그룹과-메타데이터)
- [#2.9 동적 라우터(Dynamic Routes)](/docs/routing.md/#29-동적-라우터dynamic-routes)

## [#3 데이터 패칭](/docs/data-fetching.md)

- [#3.0 소개(간단한 영화 api 첨가)](/docs/data-fetching.md/#30-소개)
- [#3.1 Client Side](/docs/data-fetching.md/#31-client-side)
- [#3.2 Server Side](/docs/data-fetching.md/#32-server-side)
- [#3.3 Loading Components](/docs/data-fetching.md/#33-loading-components)
- [#3.4 병렬 리퀘스트(Parallel Requests)](/docs/data-fetching.md/#34-병렬-리퀘스트parallel-requests)
- [#3.5 Suspense](/docs/data-fetching.md/#35-suspense)
- [#3.6 요약](/docs/data-fetching.md/#36-요약)
- [#3.7 Error Handling](/docs/data-fetching.md/#37-error-handling)
